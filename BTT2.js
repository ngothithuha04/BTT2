...
function decreaseDayAndMonth() {
  ...
  dateElement.innerHTML = `${dayOfWeek}, ${monthName} ${day}`;
...
// Add event listener to btn14
const btn14 = document.querySelector(".btn14");
btn14.addEventListener("click", decreaseDayAndMonth);
const header = document.querySelector(".header");


const headers = document.querySelectorAll('h3');

// Thay đổi màu chữ của tất cả các phần tử <h3> thành màu trắng
...
function updateDate() {
  ...
  const dayOfWeek = daysOfWeek[now.getDay()]; // Tên đầy đủ của ngày trong tuần
  const monthIndex = now.getMonth(); // Chỉ số của tháng (0-11)
  const day = now.getDate(); // Ngày của tháng
  const monthNumber = monthIndex + 1; // Chuyển đổi chỉ số của tháng thành số (1-12)
  
  dateElement.innerHTML = `${dayOfWeek}, ${monthNames[monthIndex]} ${monthNumber}`;
}
...
decreaseBtn.addEventListener("click", () => {
  if (time > 0) {
    time -= 5; // Decrease by 5 minutes
    updateDisplay();
  }
});

increaseBtn.addEventListener("click", () => {
  time += 5; // Increase by 5 minutes
  updateDisplay();
});

focusBtn.addEventListener("click", () => {
  alert(`Focus for ${time} minutes!`); // Replace with your focus function
});
